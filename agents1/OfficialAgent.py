...
class BaselineAgent(ArtificialBrain):
        ...
        while True:
            ...
            if Phase.FOLLOW_ROOM_SEARCH_PATH == self._phase:
                ...
                if self.received_messages_content and self.received_messages_content[
                    ...
                    self._answered = True
                    self._waiting = False
                    self._change_trait(trustBeliefs, self.task_names[2], "willingness", 0.2)
            ...
            if Phase.TAKE_VICTIM == self._phase:
                ...
                if len(objects) == 0 and 'critical' in self._goal_vic or len(
                    ...
                    self._waiting = False
                    # Human saves a victim together with robot
                    self._change_trait(trustBeliefs, self.task_names[2], "competence", 0.3)
    ...
    def _process_messages(self, state, teamMembers, condition, trustBeliefs):
        ...
        if self._supposed_to_remove and state['World']['nr_ticks'] - self._obstacle_found_ticks > 300:
            ...
            for msg in mssgs:
                ...
                if msg.startswith('Collect:'):
                    ...
                    if condition != 'weak' and collectVic not in self._collected_victims:
                        if self._picked_up_victim and condition != 'strong' and self.previous_msg and self.previous_msg.startswith('Collect:'):
                            self._change_trait(trustBeliefs, self.task_names[2], "competence", -0.4)
                            self._picked_up_victim = False
                ...
                if msg.startswith("Dropped"):
                    self._change_trait(trustBeliefs, self.task_names[2], "competence", 0.3)
                    self._picked_up_victim = False
                    self.received_messages = [mssg for mssg in self.received_messages if not mssg.content.startswith("Dropped")]
        ...
        with open(folder + '/beliefs/currentTrustBelief.csv') as csvfile:
            ...
            for row in reader:
                ...
                if row and row[0] != self._human_name:
                    competence = default
                    willingness = default
                    trustBeliefs[self._human_name + self.task_names[0]] = {'competence': competence, 'willingness': willingness}
                    trustBeliefs[self._human_name + self.task_names[1]] = {'competence': competence, 'willingness': willingness}
                    trustBeliefs[self._human_name + self.task_names[2]] = {'competence': competence, 'willingness': willingness}
        return trustBeliefs

    def _log_trust_belief(self, trust_beliefs, ticks):
        file_path = os.path.join(self._folder, 'beliefs/trustBeliefLog' + self._human_name + '.csv')

        # Check if the file exists to determine if we need to write headers
        file_exists = os.path.isfile(file_path)

        with open(file_path, mode='a', newline='') as csv_file:  # Open in append mode
            csv_writer = csv.writer(csv_file, delimiter=';', quotechar='"', quoting=csv.QUOTE_MINIMAL)

            # Write header only if the file does not exist
            if not file_exists:
                headers = ['ticks'] + ['name'] + [task + '_competence' for task in self.task_names] + [task + '_willingness' for task in self.task_names]
                csv_writer.writerow(headers)

            # Log trust belief values in a single row
            row = [ticks] + [self._human_name] + [trust_beliefs[self._human_name + task]['competence'] for task in self.task_names] + \
                  [trust_beliefs[self._human_name + task]['willingness'] for task in self.task_names]
            csv_writer.writerow(row)

    def _trustBelief(self, trustBeliefs, folder):
        '''
...
#             if 'Collect' in message:
        ...
        with open(folder + '/beliefs/currentTrustBelief.csv', mode='w') as csv_file:
            ...
            csv_writer.writerow([self._human_name, self.task_names[0], trustBeliefs[self._human_name + self.task_names[0]]['competence'],
                                 trustBeliefs[self._human_name + self.task_names[0]]['willingness']])
            csv_writer.writerow([self._human_name, self.task_names[1], trustBeliefs[self._human_name + self.task_names[1]]['competence'],
                                 trustBeliefs[self._human_name + self.task_names[1]]['willingness']])
            csv_writer.writerow([self._human_name, self.task_names[2], trustBeliefs[self._human_name + self.task_names[2]]['competence'],
                                 trustBeliefs[self._human_name + self.task_names[2]]['willingness']])
        return trustBeliefs

    def _change_trait(self, trustBeliefs, task_name, trait, difference):
        ...
        self._trustBelief(trustBeliefs, self._folder)
